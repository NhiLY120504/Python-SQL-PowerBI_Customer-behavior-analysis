...
rank_product AS (
	 SELECT category,
			item_purchased,
            purchase_count,
            DENSE_RANK() OVER (PARTITION BY category ORDER BY purchase_count DESC) AS ranking
	 FROM item_count
)
...
SELECT subscription_status, 
	   COUNT(*) AS count,
       ROUND(COUNT(*) * 100.0 / (SELECT COUNT(*) FROM repeat_buyers), 2) AS percentage
FROM repeat_buyers
GROUP BY subscription_status;
